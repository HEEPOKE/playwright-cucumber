...
class PlayPage {
  ...
  async searchForVideo(videoTitle: string) {
    ...
    await searchInput.type(videoTitle, { delay: 100 });
  ...
  async pauseVideo() {
    ...
    const videoElement = this.page.locator("video.video-stream");
    await videoElement.click();
  }

  async isVideoPaused() {
    const playButtonSelector = "button.ytp-play-button";
    const playButtonElement = this.page.locator(playButtonSelector);
  ...
  async selectQualityOption(expectedQuality: string) {
    const qualityOptionElement = this.page.locator(`text=${expectedQuality}`);
